...
public class AddressService {
	...
	public ResponseEntity<Address> saveAddress(Address address) {
		...
		if (data != null)
			...
			throw new AddressNotFoundException("address object has not been craeted");
	}

	public ResponseEntity<Address> findAddressById(int id) {
		Address data = addressdao.findAddressbyId(id);
		if (data != null)
			return new ResponseEntity<Address>(data, HttpStatus.FOUND);
		else
			throw new AddressNotFoundException("address object not found");
	}

	public ResponseEntity<Address> updateAddress(int id, Address address) {
		Address data = addressdao.updateAddress(id, address);
		if (data != null)
			return new ResponseEntity<Address>(data, HttpStatus.OK);
		else
			throw new AddressNotFoundException("address object not modified");
	}
	...
	public ResponseEntity<Address> deleteAddress(int id) {
		...
		if (data != null)
			...
			throw new AddressNotFoundException("address object not removed ");
	}
}
